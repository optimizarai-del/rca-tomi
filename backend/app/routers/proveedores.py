...
def update_proveedor(pid: int, data: schemas.ProveedorIn, db: Session = Depends(get_db), _: models.User = Depends(require_admin)):
...
def delete_proveedor(pid: int, db: Session = Depends(get_db), _: models.User = Depends(require_admin)):
    p = db.query(models.Proveedor).filter(models.Proveedor.id == pid).first()
    if not p: raise HTTPException(404, "Proveedor no encontrado")
